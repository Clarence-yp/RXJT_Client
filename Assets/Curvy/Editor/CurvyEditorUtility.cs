    ...
    public class CurvyEditorUtility
    {
        ...
        public static Vector3 PositionHandle(int id, Vector3 position, Quaternion rotation, float size, bool lockX, bool lockY, bool lockZ)
        {
            ...
            if (!lockZ) {
                ...
                newPos += Handles.Slider(position, rotation * Vector3.forward, sliderSize, Handles.ArrowHandleCap, snap.z) - position;
            }
            ...
            if (!lockX && !lockZ) {
                //GUI.SetNextControlName(name);
                Handles.color = new Color(0.6f, 0.9f, 0.3f);
                newPos += Handles.Slider2D(id - 1, position, rotation * new Vector3(rectSize, 0, -rectSize), rotation * Vector3.up, rotation * Vector3.right, rotation * Vector3.forward, rectSize, Handles.RectangleHandleCap, new Vector2(snap.x, snap.z)) - position;            
            }
            // Y/Z Slider
            if (!lockY && !lockZ) {
                //GUI.SetNextControlName(name);
                Handles.color = new Color(0.9f, 0.3f, 0.1f);
                newPos += Handles.Slider2D(id - 2, position, rotation * new Vector3(0, rectSize, -rectSize), rotation * Vector3.left, rotation * Vector3.up, rotation * Vector3.forward, rectSize, Handles.RectangleHandleCap, new Vector2(snap.y, snap.z)) - position;            
            }
