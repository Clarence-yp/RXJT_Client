  ...
  public partial class XCProject : System.IDisposable
  {
    ...
    public void ApplyMod(XCMod mod)
    {
      ...
      foreach (string filePath in mod.files) {
	  	//NOTE:用于确保SDK完全拷贝至xcode下，by lixiaojiang
        string[] filename = filePath.Split('#');
        bool isCopy = (filename.Length > 1) ? true : false;
        string absoluteFilePath = System.IO.Path.Combine(mod.path, filename[0]);
        string targetFilePath = absoluteFilePath;
        if (isCopy) {
          targetFilePath = ConvertUnityPathToXCodePath(mod.path, filename[0]);
          Debug.Log(string.Format("Copy File from:{0} to:{1}", absoluteFilePath, targetFilePath));
          if (targetFilePath.EndsWith(".framework")
            || targetFilePath.EndsWith(".bundle")
            ) {
            bool ret = CopyFolder(absoluteFilePath, targetFilePath, null);
            if (!ret) {
              Debug.Log(string.Format("Copy File failed! from:{0} to:{1}", absoluteFilePath, targetFilePath));
              continue;
            }
          } else {
            bool ret = CopyFile(absoluteFilePath, targetFilePath, null);
            if (!ret) {
              Debug.Log(string.Format("Copy File failed! from:{0} to:{1}", absoluteFilePath, targetFilePath));
              continue;
            }
        ...
        Debug.Log("Adding file:" + targetFilePath);
      ...
      foreach (string folderPath in mod.folders) {
        ...
        bool isCopy = (foldername.Length > 1) ? true : false;
        string absoluteFolderPath = System.IO.Path.Combine(mod.path, foldername[0]);
        string targetFolderPath = absoluteFolderPath;
        if (isCopy) {
          targetFolderPath = ConvertUnityPathToXCodePath(mod.path, foldername[0]);
          Debug.Log(string.Format("Copy Folder from:{0} to:{1}", absoluteFolderPath, targetFolderPath));
          bool ret = CopyFolder(absoluteFolderPath, targetFolderPath, (string[])mod.excludes.ToArray(typeof(string)));
          if (!ret) {
            Debug.Log(string.Format("Copy Folder failed! from:{0} to:{1}", absoluteFolderPath, targetFolderPath));
            continue;
          }
        }

        Debug.Log("Adding folder:" + targetFolderPath);
        this.AddFolder(targetFolderPath, modGroup, (string[])mod.excludes.ToArray(typeof(string)));
      }
      ...
      foreach (string headerpath in mod.headerpaths) {
        if (headerpath.Contains("$(inherited)")) {
		  ...
		  //NOTE:用于确保SDK完全拷贝至xcode下，by lixiaojiang
          string[] headername = headerpath.Split('#');
          bool isCopy = (headername.Length > 1) ? true : false;
          string absoluteHeaderPath = System.IO.Path.Combine(mod.path, headername[0]);
          string targetHeaderPath = absoluteHeaderPath;
          if (isCopy) {
            targetHeaderPath = ConvertUnityPathToXCodePath(mod.path, headername[0]);
            Debug.Log(string.Format("Convert headerpaths from:{0} to:{1}", absoluteHeaderPath, targetHeaderPath));
          }

          this.AddHeaderSearchPaths(targetHeaderPath);
        }
      }
    ...
    public string ConvertUnityPathToXCodePath(string modPath, string fileRelativePath)
    {
      return System.IO.Path.Combine(projectRootPath, fileRelativePath);
    }
    public bool CopyFolder(string srcDir, string destDir, string[] exclude)
    {
      bool ret = true;
      if (!Directory.Exists(srcDir)) {
        Debug.Log("CopyFolder failed folder not exist! folder:" + srcDir);
        return false;
      }
      if (!Directory.Exists(destDir)) {
        Directory.CreateDirectory(destDir);
      }
      string[] files = FilterFiles(srcDir, exclude);
      foreach (string curFile in files) {
        FileInfo curFileInfo = new FileInfo(curFile);
        File.Copy(curFileInfo.FullName, destDir + "/" + curFileInfo.Name, true);
      }
      string[] dirs = Directory.GetDirectories(srcDir);
      foreach (string curDir in dirs) {
        DirectoryInfo curDirInfo = new DirectoryInfo(curDir);
        ret |= CopyFolder(curDir, destDir + "/" + curDirInfo.Name, exclude);
      }
      return ret;
    }
    public bool CopyFile(string srcFile, string destFile, string[] exclude)
    {
      bool ret = true;
      if (!File.Exists(srcFile)) {
        Debug.Log("CopyFile failed file not exist! file:" + srcFile);
        return false;
      }
      if (File.Exists(destFile)) {
        Debug.Log("CopyFile Target exist! file:" + destFile);
        return true;
      }
      if (exclude != null) {
        string regexExclude = string.Format(@"{0}", string.Join("|", exclude));
        if (Regex.IsMatch(srcFile, regexExclude)) {
          Debug.Log("CopyFile failed file exclude exist! file:" + srcFile);
          return false;
        }
      }
      ...
      FileInfo curFileInfo = new FileInfo(srcFile);
      //Debug.Log(string.Format("CopyFile execute from:{0} to {1}", curFileInfo.FullName, destDir + "/" + curFileInfo.Name));
      File.Copy(curFileInfo.FullName, destDir + "/" + curFileInfo.Name, true);
      return ret;
    }
    public string[] FilterFiles(string folderPath, string[] exclude)
    {
