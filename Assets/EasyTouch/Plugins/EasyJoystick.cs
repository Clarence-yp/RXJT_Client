...
public class EasyJoystick : MonoBehaviour {
	...
	/// Message name.
	/// </summary>
	public enum MessageName{On_JoystickMoveStart,On_JoystickTouchStart, On_JoystickTouchUp, On_JoystickMove, On_JoystickMoveEnd, On_JoystickTap, On_JoystickDoubleTap};
	#endregion
  ...
  [SerializeField]
  public float MaxTouchRadiusOffset = 50f;
  ...
  public int joystickIndex = -1;
	...
	void UpdateJoystick(){
        ...
        if (joystickIndex == -1 || TouchManager.Touches.Count <= 0) {
					if (!enableSmoothing){
	...
	public void CreateEvent(MessageName message)
  {	
  ...
  void On_TouchStart(EasyTouchGesture gesture)
  {
		if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick){
          ...
          if (-1 == joystickIndex) {
            ...
            if ((gesture.position - center).sqrMagnitude < (185 * VirtualScreen.xRatio) * (185 * VirtualScreen.xRatio)) {
	...
	// Simple tap
  void On_SimpleTap(EasyTouchGesture gesture)
  {
		if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick){
			if (isActivated){
	...
	// Double tap
  void On_DoubleTap(EasyTouchGesture gesture)
  {
		if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick){
			if (isActivated){
				if (gesture.fingerIndex == joystickIndex){
  ...
  void On_TouchDown(EasyTouchGesture gesture)
  {
		...
		if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick){
			...
			if (isActivated){
				...
				Vector2 center = new Vector2( (anchorPosition.x+joystickCenter.x) * VirtualScreen.xRatio, (VirtualScreen.height-(anchorPosition.y +joystickCenter.y)) * VirtualScreen.yRatio);
				if (gesture.fingerIndex == joystickIndex){
          if (((gesture.position - center).sqrMagnitude < (185 * VirtualScreen.xRatio) * (185 * VirtualScreen.xRatio) && resetFingerExit) || !resetFingerExit) {
          ...
          //if (((gesture.position.y < Screen.height / 2 && gesture.position.x < Screen.width / 2) && resetFingerExit) || !resetFingerExit) {
            ...
            if ((joystickTouch / (zoneRadius - MaxTouchRadiusOffset - touchSizeCoef)).sqrMagnitude > 1) {
              joystickTouch.Normalize();
              joystickTouch *= (zoneRadius - MaxTouchRadiusOffset) - touchSizeCoef;
            }
          }
          else {
            On_TouchUp(gesture);
